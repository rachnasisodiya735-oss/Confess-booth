...
async function createUser({ username, password, instagramId }) {
  const id = newId();
  const createdAt = new Date().toISOString();
  if (usingPostgres) {
    const { rows } = await pool.query(
      `INSERT INTO users (id, username, password, instagram_id, created_at)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [id, username, password || null, instagramId || null, createdAt]
    );
    return toCamel(rows[0]);
  }
  const users = readJsonUsers();
  const user = { id, username, password: password || null, instagramId: instagramId || null, createdAt };
  users.push(user);
